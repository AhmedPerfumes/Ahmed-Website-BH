...
export default function ResetPasswordOTP() {
  ...
  // Helpers
  const isValidMobile = /^\d{8}$/.test(mobile);
  ...
  const handleSendOTP = async (e) => {
    ...
    if (!isValidMobile) {
      setMobileError("Please enter a valid BH mobile number (8 digits total).");
      return; // stop here
    }
          ...
          <form onSubmit={handleSendOTP} className="needs-validation">
            <div className="form-floating mb-1">
              <input
                ...
                aria-describedby="bh-mobile-help"
              />
            ...
            {mobileError && (
              <small id="bh-mobile-help" className="text-danger">
                {mobileError}
              </small>
            )}
