  ...
  const subTotalPrice = (elm) => {
    ...
    if(elm?.discount_percent) {
      ...
      // if(new Date(current_date_time) >= new Date(elm.discount.start_date) && new Date(current_date_time) <= new Date(elm.discount.end_date)) {
        // console.log('if...');
        return <td>{(((elm.price * 1.10) - ((elm.price * 1.10) / 100 * elm.discount_percent)) * elm.qty).toFixed(2)}{ currency.symbol }</td>;
      // } else {
      //   console.log('else...');
      ...
      console.log('else if');
        ...
        return <td>{(((elm.price * 1.10) - ((elm.price * 1.10) / 100 * elm.sale_price)) * elm.qty).toFixed(2)}{ currency.symbol }</td>;
    } else {
        return <td>{((elm.price * 1.10) * elm.qty).toFixed(2)}{ currency.symbol }</td>;
    }
  };
          ...
          <div className="checkout__totals">
            ...
            <table className="checkout-totals">
              <tbody>
                ...
                <tr>
                  <th>SHIPPING</th>
                  <td>{orderDetails.sub_total >= 20 ? 'You Got Free Shipping' : `Shipping Cost: ${(orderDetails.shipping_amount * 1.10).toFixed(2)}${ currency.symbol }`}</td>
                </tr>
                {/* <tr>
                  <th>SERVICE FEE</th>
                  <td>{ (orderDetails.service_amount * 1.10).toFixed(2) }{ currency.symbol }</td> 
                </tr> */}
                <tr>
                  <th>TOTAL</th>
